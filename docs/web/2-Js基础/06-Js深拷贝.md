...
```js
function cloneOtherType(targe) {
  if (isBoolean(targe) || isNumber(targe) || isString(targe) || isError(targe) || isDate(targe)) {
    const Ctor = targe.constructor;
    return new Ctor(targe);
  } else if (isRegExp(targe)) {
    return cloneReg(targe);
  } else if (isSymbol(targe)) {
    return cloneSymbol(targe);
  } else if (isFunction(targe)) {
